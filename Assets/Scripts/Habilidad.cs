...
public class Habilidad : MonoBehaviour
{
    public void DispararProyectil(Proyectil proyectil, float velocidadInicial, Vector2 direccion, int danio)
    {
        ...
        nuevoProyectil.gameObject.transform.SetParent(GameManager.instancia.contenedorDeProyectiles);

        float  anguloDeRotacion = Mathf.Atan2(direccion.y, direccion.x)*Mathf.Rad2Deg;
        nuevoProyectil.transform.Rotate(0,0,anguloDeRotacion);
